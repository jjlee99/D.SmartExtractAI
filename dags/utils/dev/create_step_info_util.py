...
STEP_INFO_DEFAULT = {
    "name":"create_step_info",
    "type":"create_step_info_step_list",
    "step_list":[
        {"name":"save","param":{"save_key":"tmp_save"}}
    ]
}

def create_step_info(data:Tuple[Any,List], input_img_type:str="file_path", output_img_type:str="file_path", step_info:dict=None, result_map:dict=None) -> Tuple[Any,List]:
    """
    오퍼레이션별 스텝 생성 함수
    ...
    :param step_info: 단계 정보 (기본값은 STEP_INFO_DEFAULT)
    ...
    return create_step_info_step_list(data=data, input_img_type=input_img_type, output_img_type=output_img_type, step_list=step_list, result_map=result_map)

def create_step_info_step_list(data:Tuple[Any,List], input_img_type:str="file_path", output_img_type:str="file_path", step_list:dict=None, result_map:dict=None) -> Any:
    """
    오퍼레이션별 스텝 생성 함수
    :param data: 이미지 파일 경로 또는 numpy 배열
    :param data_type: 입력 데이터의 타입 ("file_path", "np_bgr", "np_gray" 등)
...
def draw_block_box_xywh2(block_data:Tuple[Any,List], box_color:int=1, iter_save:bool=False, result_map:dict=None) -> tuple:
    ...
    for block_map in block_list:
        ...
        # 텍스트(한글 포함) 그리기
        draw.text((x + 3, y + 15), block_id, font=font, fill=rgb_color)
